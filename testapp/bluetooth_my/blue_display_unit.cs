...
namespace test_blue_dll
{
    ...
    {
        static public sendmessage sendmessage=null;
        /// <summary>
        ...
        /// <returns></returns>
        public static string FormatValue(IBuffer buffer, string  format)
        {
            byte[] data;
        ...
        /// <param name="format"></param>
        /// <returns></returns>
        public static IBuffer FormatData(string data, string format)
        {
            try
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < values.Length; i++)
                        bytes[i] = Convert.ToByte(values[i], (format == "DEC" ? 10 : (format == "HEX" ? 16 : 2)));
        ...
        public static string GetIdByNameOrNumber(object collection, string name)
        {
                ...
                if (int.TryParse(name.Substring(1), out devNumber))
                {
                    ...
                    if (collection is List<DeviceInformation>)
                    {
                        if (0 <= devNumber && devNumber < (collection as List<DeviceInformation>).Count)
                        {
                            ...
                            sendmessage($"Device number {devNumber:00} is not in device list range");
                    }
                    ...
                    sendmessage($"Invalid device number {name.Substring(1)}");
            }
            ...
            else
            {
                ...
                if (collection is List<DeviceInformation>)
                {
                    ...
                    {
                        sendmessage($"Can't connect to {name}.");
                    }
                    ...
                    {
                        sendmessage($"Found multiple devices with names started from {name}. Please provide an exact name." );
                    }
                }
                // for services or attributes
                else
                {
                    ...
                    {
                        sendmessage($"No service/characteristic found by name {name}.");
                    }
                    ...
                    {
                        sendmessage($"Found multiple services/characteristic with names started from { name}. Please provide an exact name.");
                    }
                }
            }
            return result;
